...
from app.store import GpuHistoryStore, GpuSnapshotStore
...
class GpuSamplerService:
    def __init__(
        self,
        *,
        snapshot_store: GpuSnapshotStore,
        history_store: GpuHistoryStore,
        sample_func: Callable[[], GpuDashboardResponse],
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="stats-gpu-sampler", daemon=True)
        self._thread.start()
        logger.info(
            "gpu sampler started acquire_interval_s=%s publish_interval_s=%s",
    ...
    def sample_once(self, *, force_publish: bool = False) -> None:
        try:
            ...
            if error_sig != self._last_error:
                logger.warning("gpu sampler sample failed: %s", error_sig)
        ...
        logger.debug(
            "gpu sampler publish #%s snapshot_ts=%s tick_ms=%s",
            self._publish_count,
            snapshot.ts,
            now_ms,
        )
