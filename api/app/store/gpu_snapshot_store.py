...
class GpuSnapshotStore:
    def __init__(self) -> None:
        self._lock = threading.Lock()
        self._snapshot: GpuDashboardResponse | None = None
        self._last_tick_ms: int | None = None

    def set_snapshot(self, snapshot: GpuDashboardResponse, tick_ms: int | None = None) -> None:
        now_ms = tick_ms if tick_ms is not None else int(time.time() * 1000)
        with self._lock:
            self._snapshot = snapshot.model_copy(deep=True)
            self._last_tick_ms = now_ms

    def get_snapshot(self) -> GpuDashboardResponse | None:
        with self._lock:
