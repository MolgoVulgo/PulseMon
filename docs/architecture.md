...
```text
Main <-> GPU <-> Weather <-> Printer (only while printer available)
```

Printer is gated by non-generated runtime code. If availability is lost while it is displayed, navigation returns to Weather.

## Data ownership

```text
Backend: Linux telemetry, API payloads, bounded in-memory histories
Firmware: Wi-Fi state, display cache, LVGL navigation, weather/news data, direct read-only printer telemetry, NVS settings
```
...
Backend telemetry uses local HTTP with JSON. OpenWeather and GNews use HTTPS with certificate validation through the ESP-IDF certificate bundle. GNews also uses an `X-Api-Key` header. Printer telemetry is a separate direct LAN path: HTTP port 80 for serial-number bootstrap followed by authenticated MQTT on port 1883 for registration and read-only status requests. No external MQTT broker is involved.
