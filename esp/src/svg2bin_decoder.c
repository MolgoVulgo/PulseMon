...
esp_err_t svg2bin_log_index_stream(FILE *fp, const char *tag)
{
    if (!fp) {
        return ESP_ERR_INVALID_ARG;
    }
    const char *log_tag = tag ? tag : "svg2bin";
    uint8_t magic[SVG2BIN_INDEX_MAGIC_LEN];
    if (fseek(fp, 0, SEEK_SET) != 0) {
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(log_tag, "svg2bin index version=%u count=%u", (unsigned)version, (unsigned)count);
    for (uint16_t i = 0; i < count; i++) {
        uint16_t entry_code = 0;
        uint8_t entry_variant = 0;
        uint8_t reserved = 0;
        ...
        (void)reserved;
        ESP_LOGI(log_tag,
                 "svg2bin entry: code=%u variant=%u offset=%lu",
                 (unsigned)entry_code,
                 (unsigned)entry_variant,
